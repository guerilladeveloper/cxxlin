...
struct vec<D,real32&>
{
	...
	vec<D,real32> operator+(const vec& rhs) const;
	vec<D,real32> vec operator-(const vec& rhs) const;
	vec operator-() const;
	vec<D,real32> operator*(real32 scalar) const;
	vec<D,real32> operator/(real32 scalar) const;
...
#include "cxxlin_vecfref.inl"

#define _CXXLIN_VECFREF_H_
#endif
